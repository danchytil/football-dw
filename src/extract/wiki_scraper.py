...
# Known Wikipedia naming issues
WIKI_NAME_FIXES = {
    "Crystal PalaceWimbledon": "Crystal Palace",
}
...
def run():
    ...
    for row in rows[1:]:
        ...
        if len(cols) >= 7:
            ...
            # Skip rows without a valid club
            if "n/a" in club_raw.lower() or "[nb" in club_raw.lower():
                continue

            # Fix known Wikipedia naming issues
            club_raw = WIKI_NAME_FIXES.get(club_raw, club_raw)
