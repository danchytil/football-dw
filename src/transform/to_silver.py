...
def run():
    ...
    try:
        # Seasons - derived from actual bronze data
        cur.execute("""
            INSERT INTO silver.seasons (season_id, start_year, end_year)
            SELECT DISTINCT season, CAST(season AS INT), CAST(season AS INT) + 1
            FROM (
                SELECT season FROM raw.api_matches
                UNION
                SELECT season FROM raw.kaggle_values
            ) all_seasons
            ON CONFLICT (season_id) DO NOTHING;
        """)
        cur.execute("SELECT array_agg(season_id ORDER BY season_id) FROM silver.seasons;")
        seasons = cur.fetchone()[0]
        print(f"Seasons {seasons} updated.")


        # Teams - source priority for display_name
        cur.execute("""
            ...
            SELECT canonical, display FROM (
                ...
                FROM (
                    ...
                    UNION ALL SELECT raw_team_name, 2 FROM raw.kaggle_values
        ...
        cur.execute("""
            ...
            FROM (
                ...
                UNION ALL SELECT raw_team_name, 'kaggle' FROM raw.kaggle_values
        ...
        # Market values - already numeric from Kaggle dataset
        cur.execute("""
        ...
        """)
        print("Market values (Kaggle) transformed and saved.")

        # Stadiums - upsert from deduplicated raw
        cur.execute("""
        ...
        """)
        print("Stadium history (Wiki) loaded.")
        ...
        # Matches - match_slug built from API tla codes
        cur.execute("""
            INSERT INTO silver.matches (match_id, season_id, match_date, home_team_id, away_team_id, home_goals, away_goals, result, match_slug)
            SELECT 
                ...
                CASE 
                    ...
                    ELSE 'D'
                END,
                TO_CHAR(CAST(r.payload->>'utcDate' AS TIMESTAMP WITH TIME ZONE), 'YYYY-MM-DD')
                    || '_' || (r.payload->'homeTeam'->>'tla')
                    || '_' || (r.payload->'awayTeam'->>'tla')
            FROM (
